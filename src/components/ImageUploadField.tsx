...
export function ImageUploadField({
  ...
  removeFieldName,
  currentUrl,
  helpText = "PNG, JPG, WEBP, atau SVG. Maks 2MB.",
  ...
  const displayUrl = markRemove ? null : preview ?? currentUrl;
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-4">
        <div
          className={`flex shrink-0 items-center justify-center overflow-hidden rounded-md border border-border bg-muted ${previewClassName}`}
        >
          {displayUrl ? (
            <Image
              src={displayUrl}
        ...
        <div className="flex-1 space-y-2">
          ...
          {currentUrl && !preview && (
            <label className="flex w-fit cursor-pointer items-center gap-1.5 text-sm text-destructive hover:underline">
              ...
              Hapus {label.toLowerCase()} saat ini
            </label>
          )}
